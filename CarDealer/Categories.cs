...
namespace CarDealer
{
    public partial class Categories : Form
    {
        ...
        {
            nameBox.Text = categoryDataGridView.SelectedRows[0].Cells[1].Value.ToString();
            ...
            {
                key = Convert.ToInt32(categoryDataGridView.SelectedRows[0].Cells[0].Value.ToString());
        ...
        private void button9_Click(object sender, EventArgs e)
        {
            if (IsValid())
            {
                ...
                else
                {
                    ...
                    {
                        conn.Open();
                        SqlCommand cmd = new SqlCommand("UPDATE category SET name = @name WHERE id = @ID", conn);
                        cmd.Parameters.AddWithValue("@name", nameBox.Text);
        ...
        private void searchBox_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                conn.Open();
                SqlCommand cmd = new SqlCommand("SELECT * FROM category WHERE name LIKE @searchText", conn);
